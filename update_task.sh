...
FileQxRe=$WorkDir/QuantumultX/haptear_cookies.conf
...
do
    ...
    if [[ -n $TaskName ]]; then
        ...
        grep -E "https://gitee\.com.+tag.+enabled" $file | perl -pe "{s|(.+\w)|\1\", \"addons\":\"https://raw.githubusercontent.com/haptear/jdauto/master/QuantumultX/haptear_cookies.conf\"\},|; s|^|    \{\"config\":\"|}" >> $FileQx
