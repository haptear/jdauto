...
[task_local]
# 京东赚赚
10 0 * * * https://raw.githubusercontent.com/haptear/jdauto/master/jd_jdzz.js, tag=京东赚赚, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jdzz.png, enabled=true

================Loon==============
[Script]
cron "10 0 * * *" script-path=https://raw.githubusercontent.com/haptear/jdauto/master/jd_jdzz.js,tag=京东赚赚

===============Surge=================
京东赚赚 = type=cron,cronexp="10 0 * * *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/haptear/jdauto/master/jd_jdzz.js

============小火箭=========
京东赚赚 = type=cron,script-path=https://raw.githubusercontent.com/haptear/jdauto/master/jd_jdzz.js, cronexpr="10 0 * * *", timeout=3600, enable=true
 */
const $ = new Env('京东赚赚');
...
!(async () => {
  ...
  console.log(`\n\n开始账号内部互助 【赚京豆(微信小程序)-瓜分京豆】活动(优先内部账号互助(需内部cookie数量大于${$.assistNum || 4}个)，如有剩余助力次数则给作者haptear助力)\n`)
  for (let i = 0; i < cookiesArr.length; i++) {
    $.canHelp = true
    if (cookiesArr[i]) {
      cookie = cookiesArr[i];
      ...
      if ($.canHelp) {
        if ($.authorTuanList.length) console.log(`开始账号内部互助 赚京豆-瓜分京豆 活动，如有剩余则给作者haptear助力`)
        for (let j = 0; j < $.authorTuanList.length; ++j) {
          console.log(`账号 ${$.UserName} 开始给作者haptear ${$.authorTuanList[j]['assistedPinEncrypted']}助力`)
